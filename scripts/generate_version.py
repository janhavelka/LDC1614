...
Standalone commands:
  sync
      Synchronize generated headers and package/documentation version metadata.
  check
      Exit with code 1 when synchronized outputs are out of date.
...
def _expected_outputs(project_root: Path) -> Dict[Path, str]:
    ...
    metadata_targets = (
        (
            ...
            r"^(?P<prefix>version:\s*)(?P<value>[^\r\n]+)$",
            "ESP-IDF component version",
        ),
        (
            project_root / "Doxyfile",
            r"^(?P<prefix>PROJECT_NUMBER\s*=\s*)(?P<value>[^\r\n]+)$",
            "Doxygen project version",
        ),
    )
    for path, pattern, label in metadata_targets:
        ...
        outputs[path] = text[:match.start("value")] + replacement + text[match.end("value"):]
