...
While this article focuses on the LDC0851 inductive switch, the coil design principles directly apply to LDC1614 sensor design. The WEBENCH Coil Designer tool can generate single-coil PCB sensors for LDC1614 applications. The fill ratio rule (dIN/dOUT < 0.3 for small targets) and the general stacked/side-by-side trade-offs inform PCB coil layout decisions when designing LDC1614-based sensing systems. Multi-layer coil designs increase inductance density, which is relevant for LDC1614 sensor frequency planning.
